...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'IkonWP_Customize_Slider_Control' ) ) {

	/**
	 * Class IkonWP_Customize_Control_Slider
	 */
	class IkonWP_Customize_Slider_Control extends IkonWP_Customize_Control {

		/**
		 * @var string
		 */
		public $type = 'ikonwp-slider';
		...
		public function to_json() {
			...
			$this->json['hide_units'] = 1 == $this->hide_units;
		}
		...
		protected function content_template() {
					...
					<span class="ikonwp-responsive-switcher">
						<# _.each( data.structures, function( setting_key, device ) { #>
							<span class="ikonwp-responsive-switcher-button preview-{{ device }}"
                                  data-device="{{ device }}">
                                <span class="dashicons dashicons-{{ 'mobile' === device ? 'smartphone' : device }}"></span>
                            </span>
						<# }); #>
					</span>
				<# } #>
            ...
            <div class="customize-control-content">
                <# _.each( data.structures, function( setting_key, device ) { #>
                <div class="ikonwp-slider-fieldset ikonwp-row {{ data.responsive ? 'ikonwp-responsive-fieldset' : '' }} {{ 'desktop' == device ? 'active' : '' }} {{ 'preview-' + device }}">
                    <div class="ikonwp-row-item" style="width: 100%;">
                        <div class="ikonwp-slider-ui"></div>
                    </div>
                    <div class="ikonwp-row-item" style="width: 50px;">
                        <input class="ikonwp-slider-input {{ data.hide_units ? '' : 'ikonwp-input-with-unit' }}"
                               type="number" value="{{ data.inputs[ setting_key ].number }}"
                    ...
                    <div class="ikonwp-row-item" style="width: 30px; {{ data.hide_units ? 'display: none;' : '' }}">
                        <select class="ikonwp-slider-unit ikonwp-unit">
                            <# _.each( data.units, function( unit_data, unit ) { #>
                            <option value="{{ unit }}"
                                    {{ unit== data.inputs[ setting_key ].unit ?
                            'selected' : '' }}
                            data-min="{{ unit_data.min }}" data-max="{{ unit_data.max }}"
                    ...
                    <input type="hidden" class="ikonwp-slider-value"
